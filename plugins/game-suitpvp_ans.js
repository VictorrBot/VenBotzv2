...
handler.before = async function (m) {
  ...
  if (room) {
    ...
    if (m.sender == room.p2 && /^(acc(ept)?|terima|gas|oke?|tolak|gamau|nanti|ga(k.)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
      ...
      m.reply(`*「✔️」Suit Telah Dikirimkan Ke Chat*
• @${room.p.split`@`[0]} 
• @${room.p2.split`@`[0]}
...
Klik wa.me/${conn.user.jid.split`@`[0]}`, m.chat, {
      ...
      if (!room.pilih) this.send3But(room.p, 'Silahkan Pilih', `Menang +${room.poin}XP\nKalah -${room.poin_lose}XP`, 'Batu✊', 'Batu', 'Kertas🖐️', 'Kertas', 'Gunting✌️', 'Gunting', m)
    ...
    if (jwb && reg.test(m.text) && !room.pilih && !m.isGroup) {
      ...
      if (!room.pilih2) this.reply(room.p2, '*Lawan Sudah Memilih✔️*\nSekarang Giliran Kamu', 0)
    }
    if (jwb2 && reg.test(m.text) && !room.pilih2 && !m.isGroup) {
      room.pilih2 = reg.exec(m.text.toLowerCase())[0]
      room.text2 = m.text
      m.reply(`Kamu Telah Memilih ${m.text} ${!room.pilih ? `\n\nMenunggu Lawan Memilih` : ''}`)
      if (!room.pilih) this.reply(room.p, '*Lawan Sudah Memilih✔️*\nSekarang Giliran Kamu', 0)
    }
